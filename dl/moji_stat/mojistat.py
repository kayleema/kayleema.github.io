...
with open(filename, 'r') as file:
    ...
    window_size = 5
    tokens = list(t.tokenize(data))
    context_dict = defaultdict(list)
    for i, token in enumerate(tokens):
        overlap = set(token.surface) & set(cjkideo)
        if overlap:
            start = max(0, i - window_size)
            end = min(len(tokens), i + window_size + 1)
            context = ''.join(t.surface for t in tokens[start:end])
            for overlap_moji in overlap:
                context_dict[overlap_moji].append(f"{token.surface} ({context})")
